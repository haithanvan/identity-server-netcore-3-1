using Nmb.Shared.Enums;
using Nmb.Shared.Linq;
...
namespace Nmb.Shared.Extensions
{
    public static class LinqExtensions
    {
        ...
        public static IEnumerable<T> SortBy<T>(this IEnumerable<T> source, params string[] sortExpression) where T : class
        {
            ...
            foreach (var t in sortExpression)
            {
                var exp = t;
                if (string.IsNullOrEmpty(exp))
                {
                ...
                var sortField = Regex.Replace(t, @"[\+\-]", string.Empty);
                var sortProperty = TypeDescriptor.GetProperties(typeof(T)).Find(sortField, true);
                source = t.StartsWith("-") ? source.OrderByDescending(a => sortProperty.GetValue(a)).ToList() : source.OrderBy(a => sortProperty.GetValue(a)).ToList();
            }
            return source;
        }
